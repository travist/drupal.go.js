  ...
  login: function(user, pass, done) {
   ...
   createContent: function(node, done) {
    ...
    if (node.fields) {
      ...
      var selector = '', info = null, lang = 'und', inst = 0, name = 'value';

      // Iterate through the fields.
      for (var field in node.fields) {

        // Get the selector value.
        info = node.fields[field];
        lang = (typeof info.language !== 'undefined') ? info.language : 'und';
        inst = (typeof info.instance !== 'undefined') ? info.instance : 0;
        name = (typeof info.name !== 'undefined') ? info.name : 'value';
        selector = info['type'];
        selector += '[name="' + field;
        selector += '[' + lang + ']';
        selector += '[' + inst + ']';
        ...
        series.push(this.go(info.action, selector, info.value));
      }
    }
